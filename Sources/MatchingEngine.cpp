...
std::vector<Trade> MatchingEngine::match() {
  ...
  while (true) {
    auto bid_opt = internal_order_book_.getTopBid();
    auto ask_opt = internal_order_book_.getTopAsk();
    ...
    internal_order_book_.cancelOrder(bid.id);
    internal_order_book_.cancelOrder(ask.id);

    if (bid.quantity > qty) {
      Order r = bid;
      r.quantity = bid.quantity - qty;
      r.timestamp = std::chrono::steady_clock::now();
      internal_order_book_.addOrder(r);
    }
    if (ask.quantity > qty) {
      Order r = ask;
